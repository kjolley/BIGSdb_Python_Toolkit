...
$(function () {
    ...
    def run(self):
        ...
        if len(eav_fields):
            print(
                '<span class="info_icon fas fa-2x fa-fw fa-globe fa-pull-left" '
                'style="margin-top:0.2em"></span>'
            )
            print("<h2>Provenance/primary metadata</h2>")
        print(
            "<p>Order columns by clicking their headings. "
    ...
    def _print_provenance_fields(self):
        ...
        td = 1
        print(
            '<table class="tablesorter" style="margin-bottom:1em"><thead>'
        ...
        for field in field_list:
            ...
            required = att["required"] if att["required"] in required_allowed else "yes"
            required_label = {
                "genome_required": "if submitting a genome assembly",
                "genome_expected": "expected if submitting a genome assembly",
            }
            value = (
                required_label[required] if required_label.get(required) else required
            )
            print(f"<td>{value}</td>")
            ...
            if field == self.system["labelfield"]:
                print(
                    f'<tr class="td{td}"><td>aliases</td>'
                ...
                print(
                    f'<tr class="td{td}"><td>references</td>'
    ...
    def _print_eav_fields(self):
        ...
        categories = self.datastore.run_query(
            "SELECT DISTINCT category FROM eav_fields ORDER BY category " "NULLS LAST",
            None,
            {"fetch": "col_arrayref"},
        )
        print(
            f'<span class="info_icon fa-2x fa-fw {icon} fa-pull-left" '
        ...
        for cat in categories:
            if categories[0] != None:
                ...
                if group_icon != None:
                    print(
                        f'<span class="subinfo_icon fa-lg fa-fw {group_icon} '
                        'fa-pull-left" style="margin-right:0.5em"></span>'
                        f'<h3 style="display:inline">{cat}</h3>'
                    )
                else:
                    print(f"<h3>{cat}</h3>") if cat != None else "<h3>Other</h3>"
                print("</div>")
            td = 1
            print(
                '<table class="tablesorter" style="margin-top:1em">'
            ...
            for field in eav_fields:
                if field.get("category"):
                    ...
                    continue
                print(
                    '<tr class="td{0}"><td>{1}</td>'
                    '<td style="text-align:left">{2}</td><td>{3}</td>'.format(
                        td,
                        field["field"],
                ...
                if field.get("option_list"):
                    ...
                    print(
                        '<td><div id="{0}" style="overflow:hidden" class="{1}">'.format(
                            field["field"], my_class
                        )
                    )
                    ...
                    if hide:
                        print(
                            '<div class="expand_link" id="expand_{0}">'
                            '<span class="fas fa-chevron-down"></span></div>'.format(
                                field["field"]
                            )
                        )
                    print("</td>")
    ...
    def _print_allowed_values(self, field):
        ...
        if att.get("optlist", "no") == "yes":
            ...
            my_class = "expandable_retracted" if hide else ""
            print(f'<div id="{field}" style="overflow:hidden" ' f'class="{my_class}">')
            ...
            if hide:
                print(
                    f'<div class="expand_link" id="expand_{field}">'
        ...
        if att.get("min") or att.get("max"):
            if att.get("min"):
                print("min: " + att["min"])
            if att.get("min") and att.get("max"):
                print("; ")
            if att.get("max"):
                print("max: " + att["max"])
            return
        ...
        if (
            field == "sender"
            or field == "sequenced_by"
            or att.get("userfield", "") == "yes"
        ):
            print(
                '<a href="{0}?db={1}&amp;page=fieldValues&amp;field=f_sender" '
                'target="_blank">Click for list of sender ids</a>'.format(
                    self.script_name, self.instance
                )
            )
            return
        ...
        if field == "curator":
            print(
                '<a href="{0}?db={1}&amp;page=fieldValues&amp;field=f_curator" '
                'target="_blank">Click for list of curator ids</a>'.format(
                    self.script_name, self.instance
                )
            )
            return
